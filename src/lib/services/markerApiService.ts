...
// ============================================================================
// LOCAL MARKER FUNCTIONS (True local - requires manual setup)
// ============================================================================
...
// ============================================================================
// CLOUD FREE MARKER FUNCTIONS (HuggingFace Spaces with free GPU)
// ============================================================================

/**
 * Submit a PDF file for conversion to Cloud Free Marker instance (HuggingFace)
 *
 * @param file - The PDF file to convert
 * @param geminiApiKey - The Gemini API key (required if use_llm is enabled)
 * @param options - Conversion options
 * @returns Submit response with request_check_url for polling
 */
export async function submitPdfConversionCloudFree(
  file: File,
  geminiApiKey: string | null,
  options: MarkerOptions
): Promise<MarkerSubmitResponse> {
  ...
  const response = await fetch(API_ENDPOINTS.MARKER_CLOUD_FREE, {
    method: 'POST',
    body: formData,
  })
  ...
  if (!response.ok || !data.success) {
    throw new Error(data.error || 'Failed to submit file for Cloud Free conversion')
  }
...
/**
 * Poll for conversion status from Cloud Free Marker instance (HuggingFace)
 *
 * @param checkUrl - The status check URL from submit response
 * @returns Poll response with status and markdown (if complete)
 */
export async function pollConversionStatusCloudFree(
  checkUrl: string
): Promise<MarkerPollResponse> {
  const response = await fetch(
    `${API_ENDPOINTS.MARKER_CLOUD_FREE}?checkUrl=${encodeURIComponent(checkUrl)}`
  )

  const data = await response.json() as MarkerPollResponse

  if (!response.ok || !data.success) {
    throw new Error(data.error || 'Failed to check Cloud Free conversion status')
  }
...
/**
 * Convert a PDF file to Markdown using Cloud Free Marker instance (HuggingFace) with automatic polling
 *
 * This is a high-level function that handles the entire conversion flow:
 * 1. Submit file
 * 2. Poll for completion
 ...
 * @returns The converted markdown content
 */
export async function convertPdfToMarkdownCloudFree(
  file: File,
  geminiApiKey: string | null,
  options: MarkerOptions,
  onProgress?: ProgressCallback,
  ...
  try {
    ...
    // Submit file
    const submitResponse = await submitPdfConversionCloudFree(file, geminiApiKey, options)

    if (!submitResponse.request_check_url) {
      return { success: false, error: 'No status check URL returned from Cloud Free Marker' }
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // Poll status
      const pollResponse = await pollConversionStatusCloudFree(checkUrl)
      ...
      if (pollResponse.status === 'complete') {
        if (!pollResponse.markdown) {
          return { success: false, error: 'No content received from Cloud Free Marker' }
        }
        return { success: true, markdown: pollResponse.markdown }
      }
      ...
      // Check if error
      if (pollResponse.status === 'error') {
        return { success: false, error: 'Conversion failed on Cloud Free Marker' }
      }
    ...
    const err = error as Error
    return { success: false, error: err.message || 'An error occurred during Cloud Free conversion' }
  }
}
